...
%used to test \ee. The hardware design of the two processor demo are
%available for download from the Evidence website, under the Literature
%section.
...
%the QAR file, and use \file{c:\\altera\\kits\\nios2\\examples\\}
%\file{\\verilog\\boardname\\evidence_2cpu} as the target directory in
%your filesystem\footnote{Of course you can select another location for
%your files. In that case, you will have to modify some directory
%locations in the OIL configuration file later on in the
%tutorial.}. \const{boardname} is the name of the board for which the
%demo was compiled for.

%The design is basically a modification of the ``standard'' Altera demo
%available under
%\file{C:\\altera\\kits\\nios2\\examples\\verilog\\niosII_stratix_1s40\\standard}. The
%Hardware design is basically a SOPCBuilder block with 2 CPUs, an input
%PIO for each processor, and an output PIO plus a mutex connected to
%all the CPU data masters.

%%
%%
%%\nb{mettere uno screenshot del sopcbuilder che fa vedere le
%%periferiche del tutorial}
%%
%%
