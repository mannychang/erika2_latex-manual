...
of concurrent applications on a single processor and on multiprocessor
systems.
...
#define OSServiceId_AlarmTick       17
#define OSServiceId_ForceSchedule   18
    \end{lstlisting}
    Please note that the primitives:
    \begin{itemize}                              \vspace{-2mm}
      \item \reffun{GetTaskID}                   \vspace{-2mm}
      ...
      \item \reffun{StartOS}                     \vspace{-2mm}
...
\begin{function}{ForceSchedule}
  ...
  \begin{fundescription}
    ...
    This function does not release the internal resource that the
    current task may have locked. This function is typically used
    after a call to \reffun{CounterTick} within a task, to force a
    reschedule of the system after a task activation that may have
    been issued within an alarm notification executed by the
    \reffun{CounterTick} primitive.
  \end{fundescription}
  \begin{funreturn}
    \fret{void}{The function is redefined as returning \const{void} when
      Standard Error is used.}  
...
linked to hardware timers). All the counters have to be defined inside
the OIL Configuration file, and the user have to call
\reffun{CounterTick} to increment them. 

Also, there is limited support for the \reftype{AlarmBaseType} type
and \reffun{GetAlarmBase} primitive. In fact, although the primitive
returns the values that are set for the alarm in the OIL file, these
values are not taken into consideration by \reffun{CounterTick},
\reffun{SetRelAlarm}, and \reffun{SetAbsAlarm}. Support for this
feature will be provided in the next versions of \ee.
\end{warning}
...
\begin{function}{CounterTick}
  ...
  \begin{fundescription}
    ...
    \reffun{Schedule} or \reffun{ForceSchedule} after the call to this
...
\begin{function}{StartOS}
  ...
  \begin{fundescription}
    ...
    The function returns to the caller after the first idle time is
    reached. It is up to the caller to implement a meaningful
    background activity. If unsure of a meaningful background
    activity, just use the \fn{for(;;)} construct.
...
\begin{function_nopb2}{OSError\_CounterTick\_AlarmID}{OSError_CounterTick_AlarmID}
  \synopsis{AlarmType OSError_CounterTick_AlarmID(void)}
  \begin{fundescription}
    The function returns the \const{AlarmID} of the Alarm notification
    triggered by \reffun{CounterTick} that raised the error. The function must
...
\begin{function_nopb2}{OSError\_CounterTick\_TaskID}{OSError_CounterTick_TaskID}
  \synopsis{TaskType OSError_CounterTick_TaskID(void)}
  \begin{fundescription}
    The function returns the \const{TaskID} related to the Alarm
    notification triggered by \reffun{CounterTick} that raised the
...
\begin{function_nopb2}{OSError\_CounterTick\_Mask}{OSError_CounterTick_Mask}
  \synopsis{EventMaskType OSError_CounterTick_Mask(void)}
  \begin{fundescription}
...
\begin{function_nopb2}{OSError\_CounterTick\_action}{OSError_CounterTick_action}
  \synopsis{EE_TYPENOTIFY OSError_CounterTick_action(void)}
  \begin{fundescription}
