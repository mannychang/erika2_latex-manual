...
not need to check directly the occurance of an event by calling a COM API primitive, 
...
configure the coomunication system support for a certain mode of operation. 
...
\reffun{StartCOM} initiliazes queued message objects with the number of receieved 
messages equal to 0. For internal transmit messages there is not any initialization 
procedure. An applicaton can directly initialize a message by \reffun{InitMessage}. If 
...
\begin{function_nopb}{StartCOMExtension}
  ...
  \begin{fundescription}
    This primitive is exucuted at the end of the \reffun{StartCOM} routine. It can
